...
# Function to check to see if a Flag should be added
Check_Flag() { eval "_FLAG=\$W$1"; if test -n "$_FLAG"; then Add_Flag "$1" "$_FLAG"; fi }

# Function to add an option '--$var1'
Add_Option() { echo "--$1 \\" >>$OUT_FILE; }

# Function to check to see if an option should be added
Check_Option() { eval "_OPT=\$W$1"; if test -n "$_OPT"; then Add_Option "$2"; fi }

# Function to add an option with a value '--$var1=$var2'
Add_Option_Val() { echo "--$1=$2 \\" >>$OUT_FILE; }

# Function to check to see if an option should be added
Check_Option_Val() { eval "_OPT=\$W$1"; if test -n "$_OPT"; then Add_Option_Val "$2"; fi }
